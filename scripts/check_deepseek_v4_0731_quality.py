#!/usr/bin/env python3
"""Short scored DeepSeek-V4-Flash-0731 reasoning and tool-use canary.

This is intentionally not a benchmark.  It verifies that a serving lane
actually enters thinking mode at high/max, returns parsed reasoning separately
from final content, and handles a few deterministic answers and one tool call.
Credentials are read from an environment variable and are never accepted as a
command-line value.
"""
...
def _payload(
    ...
    *,
    ...
    if lane == "self-hosted":
        # Make the intended mode explicit even on vLLM versions whose default
        # differs from DeepSeek's hosted contract.
...
def _score(case: QualityCase, response: dict[str, Any]) -> tuple[bool, str, int]:
    ...
    if case.expected_content is None or re.fullmatch(case.expected_content, content, re.I) is None:
...
def main() -> int:
    ...
    parser.add_argument("--lane", choices=("self-hosted", "deepseek-hosted"), default="self-hosted")
    ...
    efforts = tuple(item.strip() for item in arguments.efforts.split(",") if item.strip())
