...
export async function startHermes({ binary, publicUrl = 'http://hermes-proof.invalid', orphanReapGraceSeconds,
  bindHost = '127.0.0.1', port, source } = {}) {
  assert.ok(['127.0.0.1', '0.0.0.0'].includes(bindHost));
  if (bindHost === '0.0.0.0') {
    assert.equal(process.platform, 'linux', 'All-interface proof bind requires an isolated Linux network namespace');
    assert.ok(Object.keys(networkInterfaces()).every((name) => name === 'lo'),
      'All-interface proof bind requires a namespace with loopback only');
  }
  if (!binary) {
  ...
  if (source) {
    // The lean upstream Nix Python package omits the full CLI wrapper and UI
    // builds. Check the source against this repo's exact locked Nix input;
    // the namespace launcher resolves both from that same flake.
    const lock = JSON.parse(await readFile(new URL('../../flake.lock', import.meta.url), 'utf8'));
    assert.equal(lock.nodes['hermes-agent'].locked.rev, HERMES_REVISION);
    assert.ok((await readFile(join(source, 'hermes_cli/web_server.py'), 'utf8')).includes('_GATEWAY_WS_TICKET_PROTOCOL_PREFIX'));
  } else {
  ...
  let output = '';
  const child = spawn(binary, ['serve', '--host', bindHost, '--port', String(port), '--no-open'], {
    cwd: directory,
    env: {
      ...
      ...(source ? { HERMES_BUNDLED_PLUGINS: join(source, 'plugins'), HERMES_BUNDLED_SKILLS: join(source, 'skills'),
        HERMES_BUNDLED_LOCALES: join(source, 'locales') } : {}),
...
export async function probeAuth({ baseUrl, credentials, expiry = true, wsOrigin, rejectedOrigin, restOrigin, requestHeaders = {},
  ...
  const request = (path, options = {}) => fetch(`${base}${path}`, { redirect: 'manual', signal: AbortSignal.timeout(10000),
  ...
  const exchange = (ticket, { reject = false, origin = wsOrigin } = {}) => new Promise((resolve, rejectPromise) => {
    ...
    // Node's native client can set an Origin header; this is a protocol test,
    // explicitly not an actual browser or a production dashboard UI proof.
    const ws = new WebSocket(wsUrl, origin ? { protocols, headers: { Origin: origin } } : protocols);
    let done = false;
    let received = false;
  ...
  if (rejectedOrigin) {
    await exchange((await mint()).ticket, { reject: true, origin: rejectedOrigin });
    pass('loopback binding rejects production dashboard Origin with a valid ticket', { origin: rejectedOrigin });
  }
