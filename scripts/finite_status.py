...
def psql_query_sets(environment: dict[str, str]) -> dict[str, list[dict[str, Any]]]:
    definitions = [
        (
            ...
            "SELECT r.source_host_id,r.id,r.runtime_artifact_id,q.runner_class,q.running_creations,"
            "CASE WHEN c.creation_request_id IS NULL THEN 'missing' WHEN c.revoked THEN 'revoked' "
            "WHEN c.source_host_id<>r.source_host_id OR c.source_machine_id IS DISTINCT FROM r.source_machine_id "
            ...
            "LEFT JOIN runtime_core_credentials c ON c.agent_runtime_id=r.id "
            "LEFT JOIN LATERAL (SELECT string_agg(DISTINCT runner_class,',' ORDER BY runner_class) AS runner_class, "
            "count(*) AS running_creations FROM agent_creation_requests WHERE agent_runtime_id=r.id "
            "AND project_id=p.id AND status='running' AND owner_user_id=p.owner_user_id) q ON TRUE "
            "ORDER BY r.source_host_id,r.id;\n"
            "\\else\n"
            "SELECT r.source_host_id,r.id,r.runtime_artifact_id,q.runner_class,q.running_creations,'schema_absent' "
            "FROM agent_runtimes r JOIN projects p ON p.id=r.project_id "
            "JOIN project_runtime_links l ON l.agent_runtime_id=r.id AND l.project_id=p.id AND l.active "
            "LEFT JOIN LATERAL (SELECT string_agg(DISTINCT runner_class,',' ORDER BY runner_class) AS runner_class, "
            "count(*) AS running_creations FROM agent_creation_requests WHERE agent_runtime_id=r.id "
            "AND project_id=p.id AND status='running' AND owner_user_id=p.owner_user_id) q ON TRUE "
            "ORDER BY r.source_host_id,r.id;\n\\endif",
            ["source_host_id", "agent_runtime_id", "runtime_artifact_id", "runner_class", "running_creations", "bootstrap_state"],
        ),
        (
            "artifacts",
            ARTIFACTS_QUERY,
