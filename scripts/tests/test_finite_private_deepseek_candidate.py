...
class FinitePrivateDeepSeekCandidateTests(unittest.TestCase):
    ...
    def test_checked_in_measured_candidate_passes_release_ready_contract(self) -> None:
        self.assertEqual(check_repository(ROOT, release_ready=True), [])

    def test_prep_placeholder_fails_release_ready_contract(self) -> None:
        text = (ROOT / OFF_CANDIDATE).read_text(encoding="utf-8")
        text = VLLM_IMAGE_PATTERN.sub(VLLM_IMAGE_PLACEHOLDER, text)
        with temporary_candidate(text) as temporary_directory:
            violations = check_repository(
                Path(temporary_directory), release_ready=True
            )
        self.assertTrue(
    ...
    def test_measured_retry_image_passes_release_ready_contract(self) -> None:
        text = (ROOT / OFF_CANDIDATE).read_text(encoding="utf-8")
        text = VLLM_IMAGE_PATTERN.sub(MEASURED_IMAGE, text)
        with temporary_candidate(text) as temporary_directory:
            self.assertEqual(
