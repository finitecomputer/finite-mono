...
/// Largest NIP-05 well-known document accepted by Finite clients.
...
impl Nip05Identifier {
    ...
    ///
    /// Inputs without an `@` are treated as the NIP-05 root name for the
    /// domain, equivalent to `_@domain`.
    pub fn parse(value: &str) -> Result<Self, NostrPrimitiveError> {
        let value = value.trim();
        if value.is_empty() || value.len() > MAX_NIP05_IDENTIFIER_LEN {
            return Err(NostrPrimitiveError::LimitExceeded {
        ...
        let (local_part, domain) = match value.matches('@').count() {
            0 => ("_", value),
            1 => value
                .split_once('@')
            ...
            _ => {
                return Err(NostrPrimitiveError::MalformedInput {
                    field: "nip05_identifier",
                });
            }
        };
...
impl Nip05WellKnownDocument {
    ...
    pub fn from_json(bytes: &[u8]) -> Result<Self, NostrPrimitiveError> {
        ...
        let document: Self =
            serde_json::from_slice(bytes).map_err(|_| NostrPrimitiveError::MalformedInput {
                field: "nip05_document",
            })?;
    ...
    ) -> Result<VerifiedNip05, NostrPrimitiveError> {
        let actual_hex = self.names.get(identifier.local_part()).ok_or_else(|| {
            NostrPrimitiveError::Nip05NameMissing {
                identifier: identifier.as_str().to_owned(),
            }
        })?;
        let actual_public_key = parse_lowercase_public_key(actual_hex)?;
        if actual_public_key != expected_public_key {
            return Err(NostrPrimitiveError::Nip05PublicKeyMismatch {
                identifier: identifier.as_str().to_owned(),
    ...
    /// Resolve the public key mapped to an identifier.
    pub fn resolve(
        &self,
        identifier: &Nip05Identifier,
    ) -> Result<VerifiedNip05, NostrPrimitiveError> {
        ...
        let actual_public_key = parse_lowercase_public_key(actual_hex)?;
        self.verify(identifier, actual_public_key)
    }

    fn validate_shape(&self) -> Result<(), NostrPrimitiveError> {
...
fn validate_local_part(value: &str) -> Result<(), NostrPrimitiveError> {
...
fn normalize_domain(value: &str) -> Result<String, NostrPrimitiveError> {
...
fn parse_lowercase_public_key(value: &str) -> Result<NostrPublicKey, NostrPrimitiveError> {
    let public_key = NostrPublicKey::from_hex(value)?;
    if public_key.to_hex() != value {
        return Err(NostrPrimitiveError::Nip05KeyNotLowercaseHex {
...
fn validate_relay_urls(relays: &[String]) -> Result<(), NostrPrimitiveError> {
