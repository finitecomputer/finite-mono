...
- Removed the stale dense Brain controls from the Files sidebar; footer Brain switcher and Manage Brains are now the canonical entry points
...
- Browser verification: rebuilt Rust-served `/client` at `http://127.0.0.1:3015/client`; DOM confirmed Settings/switcher/access/invitations mounts, no legacy Brain strip, and no Access panel under `.file-sidebar`; console error list was empty
