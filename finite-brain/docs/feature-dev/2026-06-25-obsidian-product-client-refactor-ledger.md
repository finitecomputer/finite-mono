...
- Browser fixture layout: graph visible, right sidebar hidden in graph mode, vault controls collapsed after load, 47 graph nodes, 37 graph links
...
- Simplified browser fixture: target npub `npub17v7g49shev2lwp0uwrx5v88ad6hj970zfse74wkes9jguhkx7aqsgjwsvj`, page grid `44px 318px 1238px 0px`, right sidebar `display: none`, statusbar `display: none`, workspace status dots `display: none`, page header `display: none`, vault controls hidden after load, sidebar footer `11 folders / 47 pages`, graph `47 nodes / 37 links`
...
- Pure browser fixture: no hidden legacy `folderList`/`readerPageList` DOM, no duplicate Files section title, no folder count pill, no file-tree folder detail subtitles, right sidebar/status bar/page header/key footer hidden, vault controls collapsed after load, footer only `11 folders | 53 pages`, page view loads 53 readable Pages, graph view renders `53 nodes / 37 links`
...
- Collapsed vault/auth controls into a subtle disclosure after load so the file tree becomes the primary sidebar content.
...
- Final simplification pass default-opens the vault control disclosure before load, removes the hidden OKF plan list from the DOM, turns the stale OKF renderer into a no-op, and updates server asset tests to reject removed prototype copy (`Page Loop`, `OKF Import`, `Plan OKF import`).
