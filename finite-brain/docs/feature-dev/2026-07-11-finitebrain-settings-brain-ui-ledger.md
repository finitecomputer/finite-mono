# FiniteBrain Settings, Brain, And Access UI Feature Ledger

## Run

- Run ID: 2026-07-11-finitebrain-settings-brain-ui
- Loop: Feature Dev
...
Refine the FiniteBrain Product Client's settings, Brain, access-management,
...
account/Brain row, a Brain switcher with a separate Manage Brains modal, and a
...
- CONTEXT updates: pending alignment; use existing `Brain`, `Member Identity`, `Session Lock`, and `Ephemeral Client Plaintext` terms
- ADRs: none planned unless modal ownership or Brain-switch lifecycle creates a hard-to-reverse trade-off
...
- Local CodeRabbit report: `2026-07-11-settings-brain-local-coderabbit-round.md` (bounded timeout; fallback review passed)
...
- Visual verification: local Rust-served Product Client at `http://127.0.0.1:4039/client`, with desktop/mobile captures and live interaction checks for Brain switcher, settings modal, Manage Brains modal, access/sharing flows, Invitations, and Session Lock
...
| #12 Brain switcher and Manage Brains modal | AFK | complete | shared worker recovery | none | targeted checks pass |
| #13 Access & sharing in Settings | AFK | complete | staged source-anchor cleanup | none | JS/client; Rust; fmt; diff pass |
| #14 Brain invitations in Settings | AFK | complete | staged source-anchor cleanup | none | JS/client; Rust; fmt; diff pass |
...
| #12 Brain switcher and Manage Brains modal | `644e981` | `/root/ticket_12_brain_switcher` | `4442081` | standards pass; spec staged follow-up | JS/client tests; Rust test; fmt; diff |
...
| #14 Brain invitations in Settings | `18d0b10` | `/root/ticket_14_invitations_settings` | `9a80c17` | standards pass; spec staged follow-up | JS/client tests; Rust test; fmt; diff |
...
- Working decision: the bottom row owns Brain switching and Settings; Manage Brains is a dedicated modal; Access is a Settings section opened by the Access ribbon.
