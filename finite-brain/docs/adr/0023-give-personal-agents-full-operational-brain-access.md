...
organize, share, invite collaborators, and directly delete Brain content and
...
The User Nostr Identity remains the Personal Brain's sole owner. Only that human
owner may transfer or delete the Brain, change ownership or Recovery
...
A Personal Brain has exactly one Personal Agent in the current product scope.
Agent-first bootstrap may fill that role only while atomically creating the
Personal Brain; if the Brain already exists, an unpaired agent cannot enroll
itself. Personal Brains do not expose an add-another-agent flow. Multi-agent
membership and administration belong in Organization Brains, while Personal
Brain content may also be shared or exported. Personal Agent Access remains
active when the Brain contains no Folders, including after its last Folder is
deleted.
...
the Personal Brain already exists.
...
Personal Brain and adds the currently selected, identity-resolved agent as its
...
creates neither the Brain nor the Personal Agent relationship. Agent-first
bootstrap converges on the same owner, empty Brain shape, and Personal Agent
role.
...
Personal Agent returns the existing Personal Brain and relationship; it never
creates a duplicate. An unpaired or different agent fails once the Brain
exists. Any failure before the Brain and Personal Agent relationship are fully
established rolls back both so a clean retry remains possible.

Every new Personal Brain Folder automatically issues current Folder Key Grants
...
rotate every current Folder Key. The human-owned Personal Brain and its content
