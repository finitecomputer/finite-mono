# Keep Personal Brains User-Owned And Grant Agents Folder-Scoped Access
...
FiniteBrain's Personal Brain is each user's single starting place. The current
...
later Brain session without becoming a second Personal Brain owner, an
Organization Brain admin, or a holder of the user's secret.

The portable specification already permits limited Personal Brain members for
Folder-scoped sharing, but the current server/store only allow the owner to
view a Personal Brain and reject Personal Brain member mutation. The existing
...
- A user's first successful Brain initialization creates that user's one
  Personal Brain; no path creates a second. Its `ownerUserId` is the user's
...
- An Agent Principal remains a distinct Member Identity. It never becomes a
  Personal Brain owner or admin, never uses the user's Brain Identity Provider,
  and never receives the user's identity secret.
- An explicit Chat request creates a short-lived, single-use **Personal Brain
  ...
  second user-facing approval. It cannot create a second Brain or broaden
  access after setup.
...
- With that explicit delegation, Brain may add the Agent Principal as a limited
  Personal Brain Member and issue Folder Access plus encrypted Folder Key Grants
...
- The owner may later grant another restricted Folder deliberately. Project,
  ...
  access implicitly.
- A user-first bootstrap creates or ensures the user-owned Personal Brain and
  ...
  Brain, durable delegation, limited agent access, and all required Folder Key
...
- The explicit user-approved agent-first bootstrap is the only non-owner
  ...
  replayed. Once the Brain exists, only the Personal Brain owner can create,
...
- Personal Brains stay personal while providing a safe, useful starting place
  for agents.
- Server authorization and metadata filtering must allow a limited Personal
  Brain Member to see only the Personal Brain metadata and Folders in its
  explicit scope.
- Personal Brain bootstrap, member/access mutation, grant-recipient validation,
...
- Creating an Organization Brain merely to give the user's agent access.
- Making the agent a Personal Brain co-owner or admin.
- Sharing the user's Nostr secret with the agent or reviving `finite-auth`.
- Giving an agent every Personal Brain Folder by default.
