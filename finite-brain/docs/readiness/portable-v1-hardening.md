...
| --- | --- |
| Replay resistance | HTTP auth binds `u`, `method`, a fresh signed nonce, optional body payload hash, signature, and 60-second timestamp. The server rejects a reused auth event id within the configured auth window. State-changing sync records are additionally idempotent or conflict-checked by signed event id and base revision. |
