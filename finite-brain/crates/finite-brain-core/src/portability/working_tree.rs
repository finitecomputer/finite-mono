...
fn root_agents_file(brain: &Brain, actor: &UserId, acting_role: &str) -> String {
    ...
    format!(
        "# FiniteBrain {kind} Working Tree\n\nBrain ID: `{brain_id}`\nActing Member Identity: `{actor}`\nActing Brain role: `{acting_role}`\n\n## What this directory is\n\nA Finite Brain is an end-to-end encrypted knowledge base: a set of Folders whose Markdown pages sync through the FiniteBrain server as encrypted objects that only Folder Key holders can read. This directory is that Brain's Working Tree, the local decrypted mirror of every Folder the acting identity can read. Edit files here with ordinary tools; `fbrain` encrypts and syncs the changes.\n\n## Core commands (run from this directory)\n\n- `fbrain sync now --summary`: pull remote changes and push local edits.\n- `fbrain folder list`: list the Brain's Folders. Each top-level directory here is one Folder; each Markdown file inside one is a synced page.\n- Write a note by creating or editing a `.md` file under a Folder (`raw/` for captured sources, `wiki/` for durable synthesized pages), then `fbrain sync now`.\n- `fbrain wiki check`: verify internal `[[links]]` resolve.\n- `fbrain search \"<query>\"`: search every readable Folder.\n- `fbrain conflicts`: list sync conflicts that need resolution.\n\n## Sharing\n\nBrains are shared by invitation. An admin runs `fbrain invite brain create --target <email|npub>` (or the preflight/commit plan flow) and the invitee accepts with `fbrain invite brain accept --id <invitation-id>`; invitees can see incoming invitations with `fbrain invite brain list`. Folder Keys are wrapped per recipient, so a member reads only the Folders they were granted. `fbrain admin ensure-access --brain <id> --target <email>` repairs a half-onboarded member.\n\n## Rules\n\n- Read and write the materialized Folders available to this identity and role.\n- Keep non-Markdown Asset bytes outside the Brain and represent each one with a Markdown Asset Source Note under the Folder's `raw/` tree.\n- Do not write decrypted content into `.finitebrain/encrypted-sync`.\n- Do not edit generated files: this file, `_index.md`, and `_wiki/` regenerate from server state.\n- Changes must be returned through the Product Client encrypted sync path.\n\nRun `fbrain --skill` for the full agent guide to fbrain and Brain Working Trees.\n",
        brain_id = brain.id,
    )
}
