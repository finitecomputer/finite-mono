...
use finite_brain_core::{
    ...
    CoreError, CryptoRecordError, DecodedSyncPayload, DisplayName, Folder, FolderAccessMode,
...
fn normal_signed_api_router() -> Router<ServerState> {
    Router::new()
        .route(
            "/brains/{brain_id}/labels",
            get(list_principal_labels_handler),
        )
...
async fn resolve_identity_input(
    ...
    // Email-shaped targets resolve through public NIP-05 only: the Brain
    // server has no account authority to consult (auth kernel).
...
mod tests {
    ...
    async fn agent_created_organization_brain_includes_authenticated_requester() {
        ...
        .to_string();
        // The requester is declared provenance: no account authority is
        // consulted (auth kernel cut).
    ...
    async fn requester_bootstrap_rejects_unexpected_grants_without_creating_a_brain() {
        ...
        .to_string();
        let state = test_state_with_agent_owner(&agent_npub, &requester_npub);
    ...
    async fn requester_bootstrap_failure_leaves_no_organization_brain() {
        ...
        .to_string();
        let state = test_state_with_agent_owner(&agent_npub, &agent_npub);
        let router = router_with_state(state);
    ...
    async fn principal_notes_are_brain_scoped_authorized_and_separate_from_identity() {
        ...
        for (actor, brain, visible) in [
            ...
            let page: PrincipalLabelsResponse = read_json(
                authed_request(
                    router.clone(),
                    actor,
                    "GET",
                    &format!("/v1/brains/{brain}/labels"),
                    None,
                    TEST_NOW,
                )
                .await,
            ...
            .await;
            assert_eq!(page.labels.contains_key(&target), visible);
        }
        let denied = authed_request(
            router.clone(),
            &member,
        ...
        assert!(!unknown.status().is_success());
        {
            ...
            assert!(
                store
                    .load_identity_aliases(&[UserId::new(target.clone()).unwrap()])
                    .unwrap()
                    .is_empty()
            );
        }
        ...
        let receipt: PrincipalLabelReceipt = read_json(clear).await;
        assert!(receipt.label.is_none());
    }
    ...
    async fn identity_resolution_keeps_finite_vip_on_the_nip05_path() {
        ...
        // finite.vip identifiers resolve through public NIP-05 like every
        // other email-shaped input; no account authority exists to consult.
        let router = router_with_state(
            test_state().with_nip05_fixture(identifier.well_known_request().url, document),
        );
        let resolved = authed_request(
    ...
    #[tokio::test]
    async fn invitation_note_records_requested_destination_even_when_delivery_fails() {
        for failed in [false, true] {
            let admin = Keys::generate();
            let recipient = Keys::generate();
            let state = if failed {
                test_state().with_invite_mailer(|_| Err("unavailable".to_owned()))
            } else {
                test_state()
            };
            let router = router_with_state(state);
            ...
            let created: CreateBrainInviteTokenResponse = read_json(
                authed_request(
                    ...
                    "/v1/brains/acme/invite-tokens",
                    Some(
                        serde_json::json!({"role":"member", "email":"intended@example.com"})
                            .to_string(),
                    ),
                    TEST_NOW,
                )
            ...
            assert_eq!(
                ...
                if failed { "failed" } else { "not_configured" }
            );
            assert_eq!(
                authed_request(
                    router.clone(),
    ...
    async fn invite_token_email_serves_human_and_agent_readers() {
        ...
        let labels: PrincipalLabelsResponse = read_json(
            authed_request(
                router,
                &admin_keys,
                "GET",
    ...
    /// The requester flow no longer consults any account authority (auth
    /// kernel cut), so the fixture is just a clocked in-memory state.
    fn test_state_with_agent_owner(_agent_npub: &str, _owner_npub: &str) -> ServerState {
        test_state()
    }
