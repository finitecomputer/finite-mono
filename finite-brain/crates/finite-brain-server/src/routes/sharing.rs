...
pub(crate) async fn create_folder_invitation_handler(
    ...
    // The Folder Email Invite Bootstrap branch was removed with the
    // identity-proof invite flow (auth kernel cut); npub-targeted Folder
    // share links remain the only Folder invitation kind.
    if let Ok(request) = serde_json::from_slice::<CreateBrainInvitationRequest>(&body)
        && request.folder_only
    {
        return Err(ApiError::new(
            StatusCode::BAD_REQUEST,
            "email-targeted Folder invitations were removed; invite by npub, or create a capability Invite Token with email delivery",
        ));
    }
