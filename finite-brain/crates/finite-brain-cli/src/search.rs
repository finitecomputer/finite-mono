...
pub(crate) fn search<W: Write>(
    ...
    let query_admissions = acquire_search_query_admissions(&root, &folders)?;
    ...
    unlock_search_query_admissions(&query_admissions);
    result
}
...
fn select_readable_folders<'a>(
    ...
    let mut readable = tree
        ...
        .collect::<Vec<_>>();
    readable.sort_by(|left, right| folder_admission_order(left, right));
    ...
    selected.sort_by(|left, right| folder_admission_order(left, right));
    Ok(selected)
}

fn folder_admission_order(left: &WorkingTreeFolderRoot, right: &WorkingTreeFolderRoot) -> Ordering {
...
fn acquire_search_query_admissions(
    root: &Path,
    ...
    for folder in folders {
        let path = folder_index_path(root, folder);
        let lock_result = (|| {
            let lock = open_semantic_lock(&path, SEMANTIC_ADMISSION_LOCK)?;
            flock(&lock, FlockOperation::LockShared).map_err(lock_error)?;
            let lock_path = semantic_lock_path(&path, SEMANTIC_ADMISSION_LOCK)?;
            let revoked = path
                .parent()
                .is_some_and(|directory| directory.join(ACCESS_REVOCATION_MARKER).is_file());
            if !semantic_index::lock_matches_path(&lock, &lock_path) || revoked {
                let _ = flock(&lock, FlockOperation::Unlock);
                return Err(CliError::SearchIndex(
                    "Folder search admission was revoked before query execution".to_owned(),
                ));
            }
            Ok(lock)
        })();
        match lock_result {
            Ok(lock) => admitted.push(lock),
            Err(error) => {
                unlock_search_query_admissions(&admitted);
                return Err(error);
            }
        }
    }

    let current = match read_working_tree_state(root) {
        Ok(current) => current,
        Err(error) => {
            unlock_search_query_admissions(&admitted);
            return Err(error);
    ...
    let all_still_readable = folders.iter().all(|selected| {
        current.folder_roots.iter().any(|current| {
            current.folder_id == selected.folder_id
                && current.source_brain_id == selected.source_brain_id
                && current.can_read
                && !current.metadata_only
        })
    });
    if !all_still_readable {
        unlock_search_query_admissions(&admitted);
        return Err(CliError::SearchIndex(
            "Folder access changed before search results could be admitted".to_owned(),
        ));
    }
    Ok(admitted)
}
...
/// Revoke all search admission before an access-loss manifest is published.
/// The returned guards keep the transition linearized until the caller has
/// atomically replaced `working-tree-state.json`.
pub(crate) fn revoke_search_admission_before_state_publish(
    root: &Path,
    ...
    let mut revoked_folders = prior
        ...
        .collect::<Vec<_>>();
    revoked_folders.sort_by(|left, right| folder_admission_order(left, right));
...
fn hybrid_evidence(
    ...
    for folder in folders {
        ...
        if path.parent().is_some_and(|directory| {
...
fn unlock_search_query_admissions(admitted: &[File]) {
...
mod tests {
    ...
    #[test]
    fn multi_folder_search_admission_uses_one_canonical_lock_order() {
        let tree = BrainWorkingTreeStateManifest {
            version: "test".to_owned(),
            folder_roots: vec![
                WorkingTreeFolderRoot {
                    folder_id: "zeta".to_owned(),
                    source_brain_id: Some("brain-b".to_owned()),
                    path: "Mounted/Zeta".to_owned(),
                    can_read: true,
                    metadata_only: false,
                },
                WorkingTreeFolderRoot {
        ...
        let selected = select_readable_folders(&tree, &[]).unwrap();
        assert_eq!(
            selected
                .iter()
                .map(|folder| (folder.source_brain_id.as_deref(), folder.folder_id.as_str()))
