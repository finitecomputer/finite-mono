...
const statuses=Object.fromEntries(await Promise.all(['alpha','beta'].map(async site=>[site,await call({op:'site.status',site})])));
...
const sessions={};
try {
 await check('one platform deployment for both Sites',async()=>{assert.equal(statuses.alpha.deployment,statuses.beta.deployment);});
 for(const site of ['alpha','beta']){
  ...
  await check(`${site}: cross-origin and cross-site proof rejected`,async()=>{
   assert.equal((await redeem(site,proof,origins[site==='alpha'?'beta':'alpha'])).status,403);
   assert.equal((await redeem(site==='alpha'?'beta':'alpha',proof)).status,403);
  });
 ...
 await check('copied cookie and duplicate cookies cannot cross tenant gate',async()=>{
  assert.equal((await visit('beta','/',sessions.alpha)).status,403);
  assert.equal((await visit('alpha','/',`${sessions.alpha}; ${sessions.alpha}`)).status,403);
 });
 await check('forged tenant and forwarded-host headers grant no access',async()=>{
  assert.equal((await visit('alpha','/',null,{headers:{'x-tenant-id':'beta','x-finite-site':'beta','x-forwarded-host':config.controlHost}})).status,403);
 });
 await check('control mutation rejected on site host',async()=>{
  assert.equal((await visit('alpha','/api/control',sessions.alpha,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({op:'grant.set',site:'beta',email,allowed:true})})).status,405);
 });
 await check('incomplete/corrupt upload cannot replace current content',async()=>{
  const bytes=Buffer.from('Never published');
  const input={op:'version.begin',site:'alpha',commit:hash('incomplete-single-project-proof').slice(0,40),deployPath:'site',files:[{path:'index.html',sha256:hash(bytes),size:bytes.length}]};
  const begun=await call(input);assert.equal((await call(input)).version,begun.version);
  assert.equal((await api({op:'file.put',site:'alpha',version:begun.version,path:'index.html',base64:Buffer.from('corrupt').toString('base64')})).status,400);
  assert.equal((await api({op:'version.complete',site:'alpha',version:begun.version})).status,409);
  assert.equal((await api({op:'version.activate',site:'alpha',version:begun.version,expectedVersion:statuses.alpha.active_version})).status,409);
  assert.equal((await call({op:'site.status',site:'alpha'})).active_version,statuses.alpha.active_version);
 });
 const fixture=JSON.parse(await readFile(join(state,'fixture.json')));
 const old=statuses.alpha.versions.find(v=>v.source_commit===fixture.commits[0]&&v.ready).id;
 await check('rollback changes Alpha only and keeps platform deployment fixed',async()=>{
  const body={op:'version.activate',site:'alpha',version:old,expectedVersion:statuses.alpha.active_version};
  assert.equal((await api(body)).status,200);assert.equal((await api(body)).status,200);
  const r=await visit('alpha','/',sessions.alpha);assert.equal(r.status,200);assert.match(r.body,/Content version 1/);
  const beta=await visit('beta','/',sessions.beta);assert.equal(beta.headers.get('x-finite-version'),statuses.beta.active_version);
  assert.equal(r.headers.get('x-finite-deployment'),statuses.alpha.deployment);
 });
 await check('stale activation rejects lost updates; complete replay is safe',async()=>{
  assert.equal((await api({op:'version.activate',site:'alpha',version:statuses.alpha.active_version,expectedVersion:hash('stale')})).status,409);
  for(let i=0;i<2;i++)assert.equal((await api({op:'version.complete',site:'alpha',version:old})).status,200);
 });
 for(const site of ['alpha','beta']){
 ...
 await check('restoring content cannot restore revoked access',async()=>{
  await call({op:'version.activate',site:'alpha',version:statuses.alpha.active_version,expectedVersion:old});
  assert.equal((await visit('alpha','/',sessions.alpha)).status,403);
 });
 await check('disable replay and invalid disable input',async()=>{
  await call({op:'grant.set',site:'alpha',email,allowed:true});
  for(let i=0;i<2;i++)await call({op:'site.disable',site:'alpha',disabled:true});
  assert.equal((await visit('alpha','/',sessions.alpha)).status,403);
  assert.equal((await api({op:'viewer.issue',site:'alpha',verified_email:email},'issuer')).status,403);
  assert.equal((await api({op:'site.disable',site:'alpha',disabled:'false'})).status,400);
  await call({op:'site.disable',site:'alpha',disabled:false});
  assert.equal((await visit('alpha','/',sessions.alpha)).status,200);
 });
 await writeFile(join(root,'evidence/single-project-live.json'),JSON.stringify({at:new Date().toISOString(),project:config.project,deployment:statuses.alpha.deployment,origins,checks:results},null,2));
