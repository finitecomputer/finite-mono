...
Object.assign(process.env,parseEnv(await readFile(`${root}/app/.env.local`,'utf8')),{POC_DATABASE_SCHEMA:restored.schema,POC_BLOB_PREFIX:restored.blobPrefix,POC_CONTROL_HOST:config.controlHost,POC_SITE_BASE_DOMAIN:config.siteBaseDomain,POC_SITE_HOSTS:JSON.stringify(config.siteHosts)});
