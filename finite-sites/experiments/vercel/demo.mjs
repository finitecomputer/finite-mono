...
<section data-site="gamma"><h2>Managed Git + Finite identity</h2><p>Publish with a Git push. Sign in with a native Finite identity. Recover content and editable source together.</p><form method="post" action="/gamma/grant"><button>Grant native viewer</button></form><form method="post" action="/gamma/login" target="_blank"><button>Open with Finite identity</button></form><form method="post" action="/gamma/revoke"><button>Revoke native viewer</button></form><p><a href="https://github.com/alexlwn123/finite-sites-poc-source" target="_blank">Private source repository</a> · <a href="https://finite-sites-poc-native.vercel.app" target="_blank">Site URL</a></p><small>Uses isolated prototype Finite identities and the real signed-request protocol. Production account sign-in is not connected.</small></section><br><div class="sites">${['wild-alpha','wild-beta','alpha','beta'].map(site=>`<section data-site="${site}"><h2>${site}</h2><p><a href="https://${config.siteHosts[site] || `${site}.${config.siteBaseDomain}`}" target="_blank">Open URL</a></p>${[['grant','Grant access'],['login','Open private site'],['revoke','Revoke access'],['v1','Use version 1'],['v2','Use version 2']].map(([op,label])=>`<form method="post" action="/${site}/${op}" ${op==='login'?'target="_blank"':''}><button>${label}</button></form>`).join('')}<p id="${site}-summary">Loading state…</p><details><summary>Technical state</summary><pre id="${site}"></pre></details></section>`).join('')}</div>
<small>Synthetic viewer: ${email}. Operator credentials stay on this machine. Project: ${config.project}. Wildcard and alias Sites use local source fixtures; Gamma uses private managed Git.</small>
<script>fetch('/status').then(r=>r.json()).then(data=>{for(const site of ['wild-alpha','wild-beta','alpha','beta']){document.getElementById(site+'-summary').textContent='Active: version '+data[site].demoVersion+' · '+data[site].versions.filter(v=>v.ready).length+' ready versions';document.getElementById(site).textContent=JSON.stringify(data[site],null,2)}});</script>`;
createServer(async(req,res)=>{
  ...
  try {
    if(req.method==='GET'&&req.url==='/status'){
      ...
      const statuses=await Promise.all(['wild-alpha','wild-beta','alpha','beta'].map(async site=>{const status=await call({op:'site.status',site});const active=status.versions.find(v=>v.id===status.active_version);return [site,{...status,demoVersion:fixture.commits.indexOf(active?.source_commit)+1}]}));
      res.setHeader('Content-Type','application/json');return res.end(JSON.stringify(Object.fromEntries(statuses)));
    }
    const match=/^\/(wild-alpha|wild-beta|alpha|beta|gamma)\/(grant|revoke|login|v1|v2)$/.exec(req.url);
