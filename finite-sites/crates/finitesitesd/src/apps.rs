...
use finitesites_proto::limits::{MAX_APP_BUNDLE_FILES, MAX_APP_BUNDLE_UNPACKED_BYTES};

use crate::tar_safety::validate_archive_link_target;
...
const MAX_BUNDLE_ENTRIES: u32 = MAX_APP_BUNDLE_FILES;
const MAX_UNPACKED_BYTES: u64 = MAX_APP_BUNDLE_UNPACKED_BYTES;
...
fn extract_bundle(
    ...
    for entry in archive.entries()? {
        ...
        if entry_type.is_symlink() || entry_type.is_hard_link() {
            ...
            let link_name = entry
                .link_name()?
                .ok_or(AppRunnerError::BundleInvalid("link target is missing"))?;
            validate_archive_link_target(&entry_path, &link_name)
                .map_err(AppRunnerError::BundleInvalid)?;
        }
...
fn prepare_app_data_dir(data_dir: &Path) -> Result<(), AppRunnerError> {
...
mod tests {
    ...
    fn make_bundle(entries: &[(&str, &str)]) -> Vec<u8> {
        make_bundle_with_links(entries, &[])
    }

    fn make_bundle_with_links(entries: &[(&str, &str)], links: &[(&str, &str)]) -> Vec<u8> {
    ...
    fn extracts_bundle_and_applies_permissions() {
        ...
        let bundle = make_bundle_with_links(
            &[("server.js", "console.log('hi')"), ("lib/util.js", "x")],
            &[("lib/current.js", "./util.js")],
        );
        ...
        let link_meta = std::fs::symlink_metadata(release_dir.join("lib/current.js")).unwrap();
        assert!(link_meta.file_type().is_symlink());
        assert_eq!(
            std::fs::read_link(release_dir.join("lib/current.js")).unwrap(),
    ...
    fn rejects_escaping_and_empty_bundles() {
        ...
        let escaping_link = make_bundle_with_links(&[], &[("lib/outside", "../../outside.txt")]);
        let escaping_link_path = dir.path().join("escape-link.tar.gz");
        std::fs::write(&escaping_link_path, escaping_link).unwrap();
        let result = extract_bundle(
            &escaping_link_path,
            &site_dir,
            &site_dir.join("releases/v3"),
        );
        assert!(matches!(result, Err(AppRunnerError::BundleInvalid(_))));

        let absolute_link = make_bundle_with_links(&[], &[("lib/passwd", "/etc/passwd")]);
        let absolute_link_path = dir.path().join("absolute-link.tar.gz");
        std::fs::write(&absolute_link_path, absolute_link).unwrap();
        let result = extract_bundle(
            &absolute_link_path,
            &site_dir,
            &site_dir.join("releases/v4"),
        );
        assert!(matches!(result, Err(AppRunnerError::BundleInvalid(_))));
    }
    ...
    #[test]
    fn app_data_dir_prepare_is_replayable_and_does_not_chmod_contents() {
        use std::os::unix::fs::PermissionsExt as _;

        let dir = tempfile::tempdir().unwrap();
        let data_dir = dir.path().join("data");
        prepare_app_data_dir(&data_dir).unwrap();
        assert!(data_dir.is_dir());

        let state_file = data_dir.join("guestbook.db");
        ...
        std::fs::write(&file_parent, "blocks child dirs").unwrap();
        let result = prepare_app_data_dir(&file_parent.join("child"));
        assert!(matches!(result, Err(AppRunnerError::Io(_))));
    }
