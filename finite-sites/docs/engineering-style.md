...
- Mutations that clients may retry (project init, project grant/revoke/share,
  git ref reconciliation, operator disable/delete) must be idempotent or
  reject replays deterministically, and have tests for both.
...
- Record tolerated technical debt in Linear
...
- Every idempotent mutation gets success replay and conflicting-replay
  tests (project init replay, project grant/revoke/share replay, git ref
  event replay, operator disable/delete replay).
- Every storage invariant gets a restart test.
- The full project init→project grant→git push→project share→login→view loop
  has an end-to-end HTTP test that drives a real server the way the CLI, git,
  and a browser would.
