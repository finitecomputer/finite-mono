...
- A Project Repository may be Bare: zero Project Outputs, cloneable/editable
  over git, visible in Project Status/List, and not served as a website or
  other artifact.
...
- Milestone 1 Project Config supports:
  `project.slug`, output IDs, `kind = "site"`, `kind = "document"`,
  `kind = "app"`, routing names, `branch`, `path`, `entry`, `spa`, and app
  `start`. The CLI must document this schema through Agent-Safe describe
...
- `fsite describe workflow publish-stateful-app --output json` is the canonical
  first command for agents creating a stateful app Project Output. It must
  explain the same git model plus the explicit `start`, `PORT`, and `DATA_DIR`
  runtime contract.
...
- Agents edit Project Config directly and validate it with
  ...
  creating Project Repositories and Project Outputs. Replaying Project Init may
  add missing outputs to an existing Project when existing outputs match.
  Existing output config changes are intentionally not a Milestone 1 mutation;
  create a new Project/Output until an explicit update path is designed.
...
- Pushing a branch with no matching Project Output is a source-history update,
  not a failed deploy. The reconciler records/ignores the ref event without
  requiring `finite.toml` at that commit.
- `fsite project share PROJECT OUTPUT ...` is the only agent-facing viewer
  sharing mutation. It resolves the Project Output through the Project
  Repository parent, so agents do not have to switch back to a site-first
  product surface after publishing through git.
- Project Visibility controls repository read/clone/fetch access. It is
  private by default and independent from output Visibility. Selected
  Finite-owned baseline repositories, such as managed skills, may be
  public-read, but public-read never grants push access.
...
- Git smart HTTP may allow unauthenticated `git-upload-pack` only for Project
  Repositories with public-read Project Visibility. `git-receive-pack` remains
  authenticated and collaborator-gated for every Project Repository.
