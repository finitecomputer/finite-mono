...
Reconciliation is an explicit, one-off operator action, never a server-startup
side effect. The operator command defaults to a read-only preview performed on
a transactionally consistent in-memory copy of the registry. Durable changes
require `--apply yes` after the preview, backup, rollback boundary, and
production-mutation approval have been reviewed.
...
No production mutation is implied by this ADR; reconciliation must first
produce a reviewed preview report, and the same flow must be proved against
synthetic/restored state.
