...
class PinnedHermesQueueAdmissionTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_real_018_clarification_resumes_exact_chat_once(self):
        ...
        adapter._finitechat_json = fake_json
        session_key = "agent:main:finitechat:dm:room-agent-1:chat-a"
        ...
        try:
            ...
            wrong_chat = self._clarification_answer_event(
            ...
            self.assertFalse(
                any(
                    action == "ack" and payload.get("message_id") == "answer-b"
                    for action, payload, _ in bridge_calls
                )
            )

            exact = self._clarification_answer_event(
                request_id=request_id,
                segment_id="chat-a",
                seq=82,
                message_id="answer-a",
            ...
            self.assertEqual(
                [payload["message_id"] for action, payload, _ in bridge_calls if action == "ack"],
                ["answer-a"],
            )
        finally:
            clarify_gateway.clear_session(session_key)

    async def test_real_018_open_clarification_preserves_free_text(self):
        from tools import clarify_gateway
        ...
        async def fake_json(action, payload, *, timeout):
            return module._FiniteChatResult(
                True,
                {"message_id": f"{action}-ok"},
                None,
                False,
            )

        adapter._finitechat_json = fake_json
        session_key = "agent:main:finitechat:dm:room-agent-1:chat-open"
        ...
        try:
            ...
            await adapter._handle_finitechat_event(
                self._clarification_answer_event(
                    request_id=request_id,
                    segment_id="chat-open",
                    seq=83,
    ...
    @staticmethod
    def _clarification_answer_event(
        *,
        request_id: str,
        segment_id: str,
        seq: int,
        ...
        return {
            ...
            "text": text,
            "message_type": "text",
            "metadata": {"finitechat_clarification_answer": {"request_id": request_id}},
            "source": {
                "platform": "finitechat",
                "chat_id": "room-agent-1",
                "chat_type": "dm",
