...
mod engine_cutover_tripwires {
    ...
    // Retain the reader through the 2026-09-27..29 release wave (vision/stop
    // runtime image; timestamps, Brain permission repair, 200M allowance) so
    // main stays mergeable while the reader-removal PR assembles its own
    // reviewed recovery evidence; deletion is never bundled into that wave.
    const LEGACY_READER_DELETION_DEADLINE: (i64, u32, u32) = (2026, 9, 30);
