...
- Native iOS now follows ADR 0013's hard cut: device-link account setup, one
  paired agent, Home `/new`, three recent Chats, and the Topic/Chat drawer.
  Swift-owned People/relay cache, Scan/invite, direct-profile/group/member
  creation, profile/room editing, and manual-key login code and tests are
