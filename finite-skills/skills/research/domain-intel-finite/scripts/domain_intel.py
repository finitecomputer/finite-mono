...
def subdomains(domain, include_expired=False, limit=200):
    ...
    req = urllib.request.Request(
        url,
        headers={
            "User-Agent": "domain-intel-skill/1.0",
    ...
    for e in entries:
        ...
        if not include_expired and not_after:
            try:
                dt = datetime.strptime(not_after[:19], "%Y-%m-%dT%H:%M:%S").replace(
                    tzinfo=timezone.utc
                )
                if dt <= now:
                    continue
...
def check_ssl(host, port=443, timeout=10):
    ...
    return {
        ...
        "not_before": parse_date(cert.get("notBefore", "")).isoformat()
        if parse_date(cert.get("notBefore", ""))
        else "",
    ...
    for field in ("creation_date", "expiration_date", "updated_date"):
        if field in result:
            for fmt in (
                ...
                try:
                    dt = datetime.strptime(result[field][:19], fmt).replace(
                        tzinfo=timezone.utc
                    )
        ...
        if qtype == "A":
            try:
                records["A"] = list(
                    dict.fromkeys(
                        i[4][0]
                        for i in socket.getaddrinfo(domain, None, socket.AF_INET)
                    )
                )
            except Exception:
                records["A"] = []
            ...
            try:
                records["AAAA"] = list(
                    dict.fromkeys(
                        i[4][0]
                        for i in socket.getaddrinfo(domain, None, socket.AF_INET6)
                    )
                )
            except Exception:
                records["AAAA"] = []
            ...
            try:
                req = urllib.request.Request(
                    url, headers={"User-Agent": "domain-intel-skill/1.0"}
                )
...
def check_available(domain):
    ...
    try:
        ...
        req = urllib.request.Request(
            ns_url, headers={"User-Agent": "domain-intel-skill/1.0"}
        )
        with urllib.request.urlopen(req, timeout=10) as r:
    ...
    if server:
        try:
            ...
            if any(
                p in raw
                for p in ["no match", "not found", "no data found", "status: free"]
            ):
                whois_avail = True
