...
This managed baseline packages the upstream `nvk/llm-wiki` Codex skill as `llm-wiki-finite`. It does not register Claude-style custom `/wiki:*` commands or the upstream Codex plugin's `@wiki` entry point. Treat any `@wiki`, `/wiki`, `/wiki:*`, or command-flag examples in this skill and its references as shorthand for the same workflow expressed in natural language or through explicit `llm-wiki-finite` invocation.
...
When this skill activates outside of an explicit `llm-wiki-finite` invocation or `/wiki`-style shorthand:
...
4. If no relevant content → answer normally, optionally suggest: "This could be added to your wiki; ask `llm-wiki-finite` to ingest it."
