...
def get_json(
    ...
    request = urllib.request.Request(
        url,
        headers={
            "User-Agent": "finite-arxiv-finite/1.0",
...
def get_xml(params: dict[str, Any]) -> ET.Element:
    ...
    request = urllib.request.Request(
        url, headers={"User-Agent": "finite-arxiv-finite/1.0"}
    )
    with urllib.request.urlopen(request, timeout=20) as response:
...
def parse_entry(entry: ET.Element) -> dict[str, Any]:
    ...
    return {
        ...
        "summary": summary,
        "categories": [
            category.get("term") for category in entry.findall("a:category", ATOM_NS)
        ],
...
def render_entries(
    entries: list[dict[str, Any]], *, total_results: int | None = None
) -> str:
    lines: list[str] = []
    ...
    for index, entry in enumerate(entries, start=1):
        ...
        lines.append(
            f"{index}. [{entry.get('versioned_id', '?')}] {entry.get('title', '?')}"
        )
        lines.append(f"   Authors: {authors}")
        lines.append(
            f"   Published: {entry.get('published', '?')} | Updated: {entry.get('updated', '?')}"
        )
        lines.append(f"   Categories: {categories}")
        lines.append(
            f"   Abstract: {(entry.get('summary', '') or '')[:300]}{'...' if len(entry.get('summary', '') or '') > 300 else ''}"
        )
        lines.append(
            f"   Links: {entry.get('abs_url', '')} | {entry.get('pdf_url', '')}"
        )
        lines.append("")
    return "\n".join(lines).rstrip() + "\n"
...
def cmd_bibtex(args: argparse.Namespace) -> int:
    ...
    first_author_last = (
        entry["authors"][0].split()[-1] if entry.get("authors") else "paper"
    )
...
def cmd_semantic_paper(args: argparse.Namespace) -> int:
    ...
    payload = get_json(
        f"{SEMANTIC_SCHOLAR_GRAPH_URL}/paper/{semantic_scholar_id(args.id)}?fields={fields}"
    )
    if args.json:
...
def cmd_author_search(args: argparse.Namespace) -> int:
    ...
    if args.json:
        ...
        for index, author in enumerate(data, start=1):
            ...
            print(
                f"   h-index={author.get('hIndex', '?')} citations={author.get('citationCount', '?')} papers={author.get('paperCount', '?')}"
            )
            if author.get("url"):
                print(f"   {author['url']}")
            print("")
...
def cmd_recommend(args: argparse.Namespace) -> int:
    payload = get_json(
        ...
        method="POST",
        body={
            "positivePaperIds": [semantic_scholar_id(args.id)],
            "negativePaperIds": [],
        },
    )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    search.add_argument(
        "--sort", choices=["relevance", "date", "updated"], default="relevance"
    )
    ...
    semantic_paper = subparsers.add_parser(
        "semantic-paper", help="Get Semantic Scholar details for an arXiv paper"
    )
    ...
    references = subparsers.add_parser(
        "references", help="List references from a paper"
    )
    ...
    semantic_search = subparsers.add_parser(
        "semantic-search", help="Search Semantic Scholar"
    )
    ...
    author_search = subparsers.add_parser(
        "author-search", help="Search Semantic Scholar authors"
    )
    ...
    recommend = subparsers.add_parser(
        "recommend", help="Get related paper recommendations from Semantic Scholar"
    )
