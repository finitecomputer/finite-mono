...
Every file, mode, and contained workspace symlink is manifested and hashed.
...
3. Prove no host process retains a writable file descriptor or writable memory
   map below the frozen PVC, export sessions from a SQLite API snapshot, and
   seal the allow-listed bundle.
...
and restores the prior database.
