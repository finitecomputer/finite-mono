  ...
  return (
    <section className={`ocean-utility-card w-full ${formStyles.surface}`} style={{ maxWidth: 560 }}>
      ...
      <div className="grid gap-5 min-w-0">
        {!request && <form action={action} className={formStyles.form}>
          <label className={formStyles.field}>Current sign-in email<Input name="oldEmail" placeholder="current@example.com" type="email" required autoComplete="off" /></label>
          <label className={formStyles.field}>New sign-in email<Input name="newEmail" placeholder="new@example.com" type="email" required autoComplete="off" /></label>
          <label className={formStyles.field}>Support request reference<Input name="reference" required maxLength={256} placeholder="Issue or support request reference" /></label>
        ...
        {request && <div className="grid gap-3 rounded-lg border p-4 text-sm">
          ...
          {!completed && <>
            ...
            <form action={action} className={formStyles.form}>
              ...
              <label className={formStyles.field}>Code from the new mailbox<Input name="code" style={{ maxWidth: "10rem" }} inputMode="numeric" autoComplete="one-time-code" pattern="[0-9]{6}" maxLength={6} required /></label>
        ...
        <form action={action} className={`${formStyles.form} border-t pt-4`}>
          <label className={formStyles.field}>Resume an operation<Input key={request?.operationId ?? "empty"} name="operationId" placeholder="Operation ID" defaultValue={request?.operationId} required autoComplete="off" /></label>
