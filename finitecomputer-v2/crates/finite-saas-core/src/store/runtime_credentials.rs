//! Core-owned bootstrap and authenticated registration of ephemeral Iroh endpoints.
...
#[serde(rename_all = "camelCase", deny_unknown_fields)]
pub struct RuntimeEndpointRegistration {
    /// Durable per-runtime-process-start counter; retries reuse the same value.
    pub generation: i64,
...
fn new_secret() -> CoreResult<String> {
    let mut bytes = [0; 32];
...
impl RuntimeEndpointRegistration {
    pub fn validate(&self) -> CoreResult<()> {
        let url =
            reqwest::Url::parse(&self.relay_url).map_err(|_| CoreError::RuntimeSpecMismatch)?;
        if self.generation <= 0
            || !hex_secret(&self.endpoint_id)
            || self.relay_url.len() > 2048
            || url.scheme() != "https"
            || url.host_str().is_none()
            || !url.username().is_empty()
            || url.password().is_some()
...
impl CoreStore {
    ...
    pub async fn provision_runtime_credential(
        ...
        let secret = match existing {
            ...
            None => {
                ...
                tx.execute("INSERT INTO runtime_core_credentials (creation_request_id,source_host_id,bootstrap_secret,token_sha256,lease_sha256) VALUES ($1,$2,$3,$4,$5)", &[&request.id,&input.source_host_id,&secret,&digest(&secret),&lease_hash]).await.map_err(store_error)?;
                secret
            }
        };
    ...
    /// The bearer identifies the runtime; callers cannot select a Project or
    /// runtime. Locking shares the lifecycle revocation serialization boundary.
    pub async fn register_runtime_endpoint(
        &self,
        secret: &str,
        endpoint: RuntimeEndpointRegistration,
    ) -> CoreResult<Option<AuthenticatedRuntime>> {
        endpoint.validate()?;
        if !hex_secret(secret) {
            return Ok(None);
        ...
        let mut client = self.connection().await?;
        let tx = client.transaction().await.map_err(store_error)?;
        let row = tx.query_opt("SELECT creation_request_id,agent_runtime_id FROM runtime_core_credentials WHERE token_sha256=$1", &[&digest(secret)]).await.map_err(store_error)?;
        let Some(row) = row else {
            return Ok(None);
        };
        let request_id: String = row.get(0);
        let Some(runtime_id) = row.get::<_, Option<String>>(1) else {
            return Ok(None);
        };
        // Lifecycle writers acquire runtime before credential and may then
        // update creation state. Do not lock creation here in reverse order.
        let runtime = tx
            .query_opt(
        ...
        let credential = tx.query_opt("SELECT creation_request_id FROM runtime_core_credentials WHERE creation_request_id=$1 FOR UPDATE", &[&request_id]).await.map_err(store_error)?;
        ...
        let changed = tx.execute(
            "UPDATE runtime_core_credentials SET endpoint_generation=$2,endpoint_id=$3,relay_url=$4
             WHERE creation_request_id=$1 AND (endpoint_generation<$2 OR
                 (endpoint_generation=$2 AND endpoint_id=$3 AND relay_url=$4))",
            &[&request_id,&endpoint.generation,&endpoint.endpoint_id,&endpoint.relay_url],
...
mod tests {
    ...
    fn provision(request: &str) -> ProvisionRuntimeCredential {
    ...
    async fn register(db: &TestDb, request: &str) -> String {
    ...
    async fn complete(db: &TestDb, request: &str) -> CoreResult<AgentCreationLease> {
    ...
    fn endpoint(generation: i64) -> RuntimeEndpointRegistration {
        RuntimeEndpointRegistration {
            generation,
            endpoint_id: format!("{generation:064x}"),
            relay_url: "https://relay.finite.test/".into(),
        }
    }

    #[tokio::test]
    async fn creation_credential_survives_restart_and_stop_resume_without_reviving_revocation() {
        use crate::store::runtime_admissions::PeerAdmissionRequest;
        with_isolated_postgres(|db| async move {
            ...
            db.register_runtime_endpoint(&secret, endpoint(1))
                .await
                .unwrap()
                .unwrap();
            let peer = PeerAdmissionRequest {
                generation: 1,
                endpoint_id: endpoint(1).endpoint_id,
                peer_id: "a".repeat(64),
            };
            ...
            for (index, stop) in [false, true, false, false].into_iter().enumerate() {
                ...
                if stop {
                    assert!(
                        db.runtime_admissions(&secret, 1, &peer.endpoint_id)
                            .await
                            .unwrap()
                            .is_none()
                    );
                    ...
                    // Stop removed outstanding admissions, but kept the creation's
                    // enablement and secret. A new endpoint must be admitted anew.
                    assert!(
                        db.runtime_admissions(&secret, 1, &peer.endpoint_id)
                            .await
                            .unwrap()
                    ...
                    db.register_runtime_endpoint(&secret, endpoint(2))
                        .await
                        .unwrap()
                        .unwrap();
                    assert!(
                    ...
                    assert!(
                        db.runtime_admissions(&secret, 2, &endpoint(2).endpoint_id)
                            .await
                            .unwrap()
                            .unwrap()
                            .peers
                    ...
                    let current = PeerAdmissionRequest {
                        generation: 2,
                        endpoint_id: endpoint(2).endpoint_id,
                        peer_id: peer.peer_id.clone(),
                    };
                    db.admit_runtime_peer(&project, "runtime-auth-user", false, &current)
                        .await
                        .unwrap();
                }
            }
    ...
    #[tokio::test]
    async fn runtime_admissions_are_owner_scoped_default_off_and_endpoint_bound() {
        use crate::store::runtime_admissions::PeerAdmissionRequest;
        with_isolated_postgres(|db| async move {
            let request = requested(&db).await;
            let secret = db
            ...
            let runtime = register(&db, &request).await;
            let identity = db
                .register_runtime_endpoint(&secret, endpoint(1))
                .await
                .unwrap()
                .unwrap();
            assert!(
                db.runtime_admissions(&secret, 1, &endpoint(1).endpoint_id)
                    .await
                    .unwrap()
                    .unwrap()
                    .peers
            ...
            assert!(
                db.admit_runtime_peer(&identity.project_id, "runtime-auth-user", false, &peer)
                    .await
                    .is_err()
            );
            db.set_runtime_hosted_access(&identity.project_id, &peer, true)
                .await
                .unwrap();
            assert!(
                db.admit_runtime_peer(&identity.project_id, "other-user", false, &peer)
                    .await
                    .is_err()
            );
            db.admit_runtime_peer(&identity.project_id, "runtime-auth-user", false, &peer)
            ...
            db.remove_runtime_peer(&identity.project_id, "runtime-auth-user", false, &peer)
                .await
                .unwrap();
            assert!(
                db.runtime_admissions(&secret, 1, &peer.endpoint_id)
            ...
            db.admit_runtime_peer(&identity.project_id, "runtime-auth-user", false, &peer)
                .await
                .unwrap();
            db.set_runtime_hosted_access(&identity.project_id, &peer, false)
                .await
                .unwrap();
            assert!(
                db.runtime_admissions(&secret, 1, &peer.endpoint_id)
    ...
    async fn wait_for_blocker(db: &TestDb, blocker: i32) {
        tokio::time::timeout(std::time::Duration::from_secs(5), async {
            loop {
                let waiting: bool = db.connection().await.unwrap().query_one(
                    "SELECT EXISTS(SELECT 1 FROM pg_stat_activity WHERE $1=ANY(pg_blocking_pids(pid)))", &[&blocker],
                ).await.unwrap().get(0);
                if waiting { break; }
                tokio::time::sleep(std::time::Duration::from_millis(10)).await;
            }
        }).await.expect("operation must actually wait on the held row lock");
    }

    #[tokio::test]
    async fn admission_snapshot_racing_stop_returns_unauthorized_not_superseded() {
        with_isolated_postgres(|db| async move {
            let request = requested(&db).await;
            let secret = db
                .provision_runtime_credential(provision(&request))
            ...
            let task = tokio::spawn(async move {
                store
                    .runtime_admissions(&secret, 1, &endpoint(1).endpoint_id)
                    .await
            });
            wait_for_blocker(&db, pid).await;
            // Execute the real lifecycle writer under its held runtime lock.
            super::super::apply_runtime_control_completion(
                &*tx,
                &runtime,
    ...
    #[tokio::test]
    async fn runtime_bootstrap_lock_wait_cannot_extend_provisioning_lease() {
        with_isolated_postgres(|db| async move {
            let request=requested(&db).await;
            let mut client=db.connection().await.unwrap();
            let tx=client.transaction().await.unwrap();
            let pid: i32=tx.query_one("SELECT pg_backend_pid()",&[]).await.unwrap().get(0);
            tx.execute("UPDATE agent_creation_requests SET lease_expires_at=clock_timestamp()+INTERVAL '1 second' WHERE id=$1", &[&request]).await.unwrap();
            let input=provision(&request);let store=db.store.clone();
            let task=tokio::spawn(async move {store.provision_runtime_credential(input).await});
            wait_for_blocker(&db,pid).await;
    ...
    #[tokio::test]
    async fn runtime_bootstrap_registration_waits_for_lifecycle_without_lock_inversion() {
        with_isolated_postgres(|db| async move {
            let request = requested(&db).await;
            let secret = db
                .provision_runtime_credential(provision(&request))
            ...
            let task =
                tokio::spawn(
                    async move { store.register_runtime_endpoint(&secret, endpoint(1)).await },
                );
            wait_for_blocker(&db, pid).await;
            tx.batch_execute("SET LOCAL lock_timeout='500ms'")
                .await
                .unwrap();
            // Upgrade completion updates creation state after holding runtime.
            tx.execute(
                "UPDATE agent_creation_requests SET updated_at=clock_timestamp() WHERE id=$1",
                &[&request],
            )
            .await
            .unwrap();
            ...
            .unwrap();
            tx.commit().await.unwrap();
            // In-place upgrade keeps the creation's credential; registration
            // resumes after the writer commits without a lock inversion.
    ...
    #[tokio::test]
    async fn runtime_bootstrap_endpoint_retries_stale_generations_and_revocation() {
        with_isolated_postgres(|db| async move {
            let request=requested(&db).await;
            let secret=db.provision_runtime_credential(provision(&request)).await.unwrap().secret;
            let runtime=register(&db,&request).await;
            for value in [endpoint(1),endpoint(1),endpoint(2)] {
                assert!(db.register_runtime_endpoint(&secret,value).await.unwrap().is_some());
            }
            assert!(db.register_runtime_endpoint(&secret,endpoint(1)).await.is_err());
            let mut conflict=endpoint(2);conflict.endpoint_id=endpoint(3).endpoint_id;
            assert!(db.register_runtime_endpoint(&secret,conflict).await.is_err());
            let rows=db.query_json("SELECT jsonb_build_object('generation',endpoint_generation) FROM runtime_core_credentials WHERE creation_request_id=$1", &[&request]).await;
            assert_eq!(rows[0]["generation"],2);
            db.revoke_runtime_credential(&runtime,&request).await.unwrap();
            assert!(db.register_runtime_endpoint(&secret,endpoint(3)).await.unwrap().is_none());
            assert!(db.provision_runtime_credential(provision(&request)).await.is_err());
        }).await;
    }
    ...
    #[tokio::test]
    async fn runtime_bootstrap_real_http_issuance_and_self_registration() {
        ...
        with_isolated_postgres(|db| async move {
            ...
            let url =
                format!("{base}/api/core/v1/agent-creation-requests/{request}/runtime-credential");
            let body =
                serde_json::json!({"runnerId":"auth-runner","leaseToken":"test-launch-lease"});
            for bearer in ["service", "usage", "unknown"] {
                assert_eq!(
                    client
                        .post(&url)
                        .bearer_auth(bearer)
            ...
            let secret = response
                ...
                .unwrap()
                .secret;
            let queue_url = base.clone();
            let queue_request = request.clone();
            let replay = tokio::task::spawn_blocking(move || {
                use finite_saas_runner::AgentCreationQueue;
                let mut queue =
                    finite_saas_runner::CoreHttpAgentCreationQueue::new(queue_url, "runner-secret")
                        .unwrap();
                queue
                    .provision_runtime_core_credential(
                        &queue_request,
                        "auth-runner",
                        "test-launch-lease",
                    )
            ...
            .await
            .unwrap();
            assert!(secret == replay.secret);
            let runtime = register(&db, &request).await;
            let url = format!("{base}/api/core/v1/runtime/iroh-endpoint");
            for bearer in ["runner-secret", "service", "usage"] {
                assert_eq!(
                    client
                        .post(&url)
                        .bearer_auth(bearer)
            ...
            use finite_agentd::{CoreEndpointRegistration, EndpointRegistrationOutcome, Ledger};
            let directory = tempfile::tempdir().unwrap();
            let path = directory.path().join("agentd.sqlite3");
            let ledger = Ledger::open(&path).unwrap();
            let first = CoreEndpointRegistration::new(
                &base,
                secret.clone(),
                &ledger,
                endpoint(1).endpoint_id,
                endpoint(1).relay_url,
            )
            .unwrap();
            assert_eq!(
                first.register().await.unwrap(),
                EndpointRegistrationOutcome::Accepted
            );
            assert_eq!(
                first.register().await.unwrap(),
                EndpointRegistrationOutcome::Accepted
            ...
            let second = CoreEndpointRegistration::new(
                &base,
                secret.clone(),
                &reopened,
                endpoint(2).endpoint_id,
                endpoint(2).relay_url,
            )
            .unwrap();
            assert_eq!(
            ...
            assert_eq!(
                first.register().await.unwrap(),
                EndpointRegistrationOutcome::Superseded
            );
            db.revoke_runtime_credential(&runtime, &request)
                .await
                .unwrap();
            assert_eq!(
                second.register().await.unwrap(),
                EndpointRegistrationOutcome::Unauthorized
            );
            server.abort();
            let _ = server.await;
        })
    ...
    mod runtime_iroh_proof {
        include!("runtime_iroh_proof.rs");
    }
}
