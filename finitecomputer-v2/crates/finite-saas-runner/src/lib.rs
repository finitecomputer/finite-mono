...
mod tests {
    ...
    fn bundled_finitebrain_skill_uses_runtime_brain_and_durable_paths() {
        ...
        assert!(bundled_creation.contains("turn-scoped requester lease"));
        assert!(bundled_creation.contains("fbrain brain create organization \"$NAME\" --json"));
        assert!(!bundled_creation.contains("--requesting-user-npub"));
        ...
        assert!(bundled.contains("inspect the skills that are actually installed"));
        assert!(bundled.contains("authoritative primary documentation"));
        assert!(bundled.contains("Write only sourced synthesis under `wiki/`"));
        assert!(bundled.contains("Do not silently substitute model knowledge"));
    ...
    fn runner_binary_advertises_one_worker_class_without_product_backend_switching() {
        ...
        assert!(main_rs.contains(r#"required_env("FC_RUNNER_CLASS")"#));
        assert!(main_rs.contains(r#""kata" =>"#));
        assert!(main_rs.contains(r#""apple_container" =>"#));
        assert!(!main_rs.contains(r#""local_docker""#));
        assert!(!main_rs.contains(r#""enclavia""#));
        assert!(!main_rs.contains(r#""phala""#));
