---
name: finite-v2
description: Apply Finite Computer v2 product boundaries when changing Core, Runner, dashboard, Agent Runtime orchestration, or the runtime-local finite utility. Read before editing these components.
---

# Finite Computer v2 boundaries
...
- Keep Hermes pinned through the root flake across image, smoke, and release
  ...
  `finitecomputer-v2/deploy/finite-computer/images/runtime.Dockerfile`; never hand-copy one. Baseline agent CLIs (Node from Hermes, bun,
  deno, uv, Playwright browsers, and the weasyprint HTML→PDF CLI) are
  `.#agent-runtime-toolchains` on the
  same flake (`finitecomputer-v2/deploy/finite-computer/images/agent-runtime-toolchains.nix`);
...
Core lives in `finitecomputer-v2/crates/finite-saas-core/`. Its `src/lib.rs`
re-exports domain types, `src/api.rs` assembles routes, and `src/store.rs` owns
connection/migration setup. Open the relevant child module instead of reading
the whole crate. Before changing persistence, read `PERSISTENCE.md` in the
crate root and prove writes against real Postgres. Keep wire shapes, public
exports, and migration SQL stable during structural cleanup. Run
`just source-structure-check`; its scope and exceptions are documented in
`docs/agents/source-structure.md`.

## Prompting contract

For substantial tasks, help the user establish a self-contained problem,
acceptance criteria, constraints (musts, must-nots, preferences, escalation
points), clean phases, and an evaluation design. Infer routine details from
the request and existing contracts; clarify missing information that affects
the outcome.
