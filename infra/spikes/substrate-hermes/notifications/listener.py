...
WAKE_READY=threading.Condition(LOCK)
BUSY=set();RETRY={};POOL=ThreadPoolExecutor(max_workers=8)
def event(owner):
    with LOCK:
        ...
        WAKE_READY.notify_all()
    print(json.dumps({'notification':owner,'t':time.time()}),flush=True)
def subscriptions():
...
def wake_one(owner,since):
    try:
        cfg=OWNERS[owner]
        headers={'ate-target-actor':cfg['actor'],'x-simplex-wake':cfg['token']}
        req=urllib.request.Request(cfg['wake_url'],headers=headers)
        print(json.dumps({'wake_start':owner,'t':time.time()}),flush=True)
        with urllib.request.urlopen(req,timeout=120) as response:
            if response.status!=200:raise RuntimeError('Wake HTTP failure')
        with LOCK:
            ...
            RETRY.pop(owner,None)
        print(json.dumps({'wake':owner,'t':time.time()}),flush=True)
    except Exception as e:
        with LOCK:RETRY[owner]=time.monotonic()+1
        print(json.dumps({'wake_retry':owner,'error':type(e).__name__}),flush=True)
    finally:
        with WAKE_READY:
            BUSY.remove(owner);WAKE_READY.notify_all()
def wake():
    # Pending SQLite rows are authoritative; notifications merely wake this
    # scheduler. Re-read them after every condition wake and on process start.
    with WAKE_READY:
        while not STOP.is_set():
            now=time.monotonic()
            pending=list(DB.execute('SELECT owner,since FROM pending'))
            for owner,since in pending:
                if owner in BUSY or RETRY.get(owner,0)>now or len(BUSY)>=8:continue
                BUSY.add(owner);POOL.submit(wake_one,owner,since)
            due=[RETRY[o]-now for o,_ in pending if o not in BUSY and RETRY.get(o,0)>now]
            WAKE_READY.wait(timeout=min(due) if due else None)
