...
p=argparse.ArgumentParser();p.add_argument('--state-dir',type=Path,required=True);p.add_argument('--image',required=True);p.add_argument('--key-file',type=Path,required=True);a=p.parse_args()
ns='finite-hermes-spike';name='simplex-notifications';owners={}
def write(name,obj):
...
for user in ('alice','bob'):
 actor=user+'-notify';token=old.get(user,{}).get('token') or secrets.token_urlsafe(32)
...
for item in ingress['items']:
 if item['kind']=='ConfigMap':item['data']['nginx.conf']=item['data']['nginx.conf'].replace('/alice-agent','/alice-notify').replace('/bob-agent','/bob-notify')
