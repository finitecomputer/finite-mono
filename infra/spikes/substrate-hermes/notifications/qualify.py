...
a=p.parse_args()
owner=next(o for o in json.loads(Path(__file__).with_name('owners.json').read_text()) if o['user_id']==a.user)
...
c.login()
contacts=[x for x in human_contacts() if x.get('localDisplayName')==a.user+'-notify']
if not contacts:
