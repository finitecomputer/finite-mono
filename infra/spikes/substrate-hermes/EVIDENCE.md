> Superseded architecture: the external per-agent transport results below are
> historical. The current proof uses [one shared notification-only listener](notifications/README.md),
> with both SimpleX clients inside their sleeping actors. Its evidence is in
> `private-*.json` and `notification-*.json` alongside the earlier reports.
