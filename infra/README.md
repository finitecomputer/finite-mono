...
2026-07-09 reinstall transcript was retired 2026-08-29 (runbook
consolidation — its hard-won facts live in
[`runbooks/incident.md`](runbooks/incident.md) §5), and destructive reuse is paused while
...
infra/
  ...
  runbooks/    # three survivors: release, incident, recovery (+ in-flight cutover records)
```
...
inventory in `hosts/lat2/runners.md` is removal inventory for the lat2
cutover's Gate E cleanup; mono Docker/image CI is no longer scheduled
there.
...
1. **The app-plane host runs exact NixOS closure activation from a release
   rev.** The rev that
   ...
   truth: `infra/nixos/`. There is no current bare-metal wipe authority;
   host rebuilds follow [`runbooks/incident.md`](runbooks/incident.md) §4–§5.
...
(Moved from the former runbook index, 2026-08-29.)

- **One status command:** run `scripts/finite-status` before and after every
  rollout; use `--json` for retained evidence or automation. It is read-only
  and exits `0` when all sections are green, `1` when any section is red,
  and `2` when a result could not be determined and nothing is already red.
  Its full contract (what it observes, root requirements for the
  lifecycle-probe fields, exit codes) is documented in
  `scripts/finite_status.py`; if an incident tempts you to type an ad-hoc
...
- Nothing is built on a prod box. Images are CI-built, digest-pinned, from
  `infra/images/` (deploy principle 2).
- Never open a snapshot SQLite file directly; inspect it only through
  `scripts/snapshot-sqlite` (private scratch copy) or from a scratch copy.
- Any manual change made on a box during an incident must land back in
  `infra/` (or be reverted) **within a day** —
  [`runbooks/incident.md`](runbooks/incident.md) §1.
- Rust service packages use content-scoped sources in
  `infra/nixos/packages.nix`; see that file's header for the scoping rules
  and the `Nix service packages` CI lane that enforces them.
- Backups are only real once restored (deploy principle 5); the drills and
  their current honest status live in
  [`runbooks/recovery.md`](runbooks/recovery.md) §3.
