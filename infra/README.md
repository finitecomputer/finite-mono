...
| **finite-lat-3** (207.188.7.157) | **NixOS 26.05 storage-qualified, drained Agent Runner candidate.** Kernel 6.18.39; 187 GiB RAM; exact-size RAID1 root and `/data`; dual ESPs; 64-GiB swapfile plus zswap. | Installed 2026-07-20 from pinned input `fd14620`; Runner/Kata closure from merged PR #131 is active. Containerd is active with zero containers. The Runner timer/service are inactive and the required environment is absent. The lat3 WireGuard peer is up but cannot handshake before the bounded lat1 peer/proxy activation. No user Agent or Recovery Authority exists here. |
