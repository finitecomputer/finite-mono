...
| Host | Role | Services |
|---|---|---|
| **lat1** (64.34.82.77) | **Consolidated NixOS app server** (`infra/nixos/`) | finite-saas-core (:4200), dashboard (podman :3000), **native** Postgres 16 (`services.postgresql`, `finite_core`, 87 FP keys), finitechat-server (:8788), finitesitesd (:8787), finite-search (SearXNG :8080 + Firecrawl), finite-saas-runner (Kata timer enabled/active at read-only preflight 2026-07-10T19:03Z, but selected artifact `.5` returned `503` from `/healthz` and `/contact`, and the live runner lacks `FC_CORE_RUNNER_API_TOKEN`; canary readiness is **BLOCKED**), **one** Caddy edge for `finite.computer` + `chat.finite.computer` + `*.finite.chat` + `*.docs.finite.chat`. NO k3s, NO Traefik, NO on-host image builds. Deploy: `nixos-rebuild --flake ...#finite-lat-1`. |
