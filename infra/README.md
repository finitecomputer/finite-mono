...
| **finite-lat-3** (207.188.7.157) | **NixOS 26.05 storage-qualified, drained Agent Runner candidate.** Kernel 6.18.39; 187 GiB RAM; exact-size RAID1 root and `/data`; dual ESPs; 64-GiB swapfile plus zswap. | Installed 2026-07-20 from pinned input `fd14620`. Full member replacement, degraded reboot, checksum, ESP guard, and swap checks pass. The next bounded change adds the private lat1 peer plus Kata/Runner with creation drained and its timer disabled. No user Agent or Recovery Authority exists here yet. |
