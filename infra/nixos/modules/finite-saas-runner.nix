...
}:
{
  ...
  # hosts that import identity (the lat2 app plane) would get
  # ExecStart-less husk units that violate the app-plane no-runner
  # contract enforced by deploy-lat2-closure-cache.
  options.finite.saasRunner = lib.mkOption {
    type = lib.types.bool;
    default = false;
    description = "True exactly on hosts that import the runner worker module.";
  };

  config = {
    finite.saasRunner = true;
