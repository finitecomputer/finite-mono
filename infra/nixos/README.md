...
| `/etc/finite-saas/gate.env` | `FINITE_GATE_SIGNING_KEY` (64 lowercase hex, `openssl rand -hex 32`), `FINITE_GATE_ACCOUNT_TOKEN` | the Auth Gate (`finite-gated`, `auth.finite.site`) vouch-signing key and server-to-server account credential. The gate logs the key's public counterpart at startup; pin that hex on Sites as `FINITE_SITES_AUTH_GATE_PUBKEY`. Store the key in the team password manager; the account credential must also be configured on the dashboard server; it is never sent to browsers. |
| `/etc/finite/sites-auth-gate.env` | `FINITE_SITES_AUTH_GATE_URL`, `FINITE_SITES_AUTH_GATE_PUBKEY` | non-secret gate wiring for finitesitesd: the gate's public origin (`https://auth.finite.site`) and the pinned vouch-verification public key (the counterpart of the gate's `FINITE_GATE_SIGNING_KEY`; 64 lowercase hex). The two names must be set together. |
