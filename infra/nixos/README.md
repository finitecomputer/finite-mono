...
| `/etc/finite-saas/gate.env` | `FINITE_GATE_SIGNING_KEY` (64 lowercase hex, `openssl rand -hex 32`), `FINITE_GATE_WORKOS_CLIENT_ID`, `FINITE_GATE_WORKOS_API_KEY` | the Auth Gate (`finite-gated`, `auth.finite.computer`) vouch-signing key and WorkOS AuthKit client. The gate logs the key's public counterpart at startup; pin that hex on Sites as `FINITE_SITES_AUTH_GATE_PUBKEY`. Store the key in the team password manager. The gate refuses to start without the WorkOS pair — dev mode is an explicit `FINITE_GATE_DEV_MODE=1` opt-in, never a fallback. |
| `/etc/finite/sites-auth-gate.env` | `FINITE_SITES_AUTH_GATE_URL`, `FINITE_SITES_AUTH_GATE_PUBKEY` | non-secret gate wiring for finitesitesd: the gate's public origin (`https://auth.finite.computer`) and the pinned vouch-verification public key (the counterpart of the gate's `FINITE_GATE_SIGNING_KEY`; 64 lowercase hex). The two names must be set together. |
