...
| `/etc/finite/grafana-cloud-metrics.env` | `GRAFANA_CLOUD_PROMETHEUS_URL`, `GRAFANA_CLOUD_PROMETHEUS_USERNAME`, `GRAFANA_CLOUD_PROMETHEUS_PASSWORD` | Install the same root-owned, mode `0600` file independently on finite-lat-1 and finite-lat-3. Copy the remote-write URL and Prometheus username from the Grafana Cloud stack details. Create a stack-scoped access-policy token with only `metrics:write` for the password. This file is read only by Grafana Alloy and must exist before activating a closure that enables Alloy. |
