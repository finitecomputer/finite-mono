...
let
  ...
  storageHealth = pkgs.writeShellApplication {
    ...
    text = ''
      ...
      # Exact component_size values carried over from the finite-lat-3
      # qualification for the same disk classes; re-prove them against the
      # captured physical disks at Gate B before this host is installed.
...
in
{
  systemd.services = {
    finite-prepare-data-root = {
      description = "Prepare bounded finite-lat-2 data roots on the exact data filesystem";
      requires = [ "data.mount" ];
      after = [ "data.mount" ];
    ...
    finite-storage-health = {
      description = "Fail-closed finite-lat-2 storage, ESP, swap, and SMART health";
      after = [
        "local-fs.target"
        "swap.target"
      ];
    ...
    finite-md-check = {
      description = "Serialized finite-lat-2 MD consistency check";
      serviceConfig = {
        Type = "oneshot";
