...
in
{
  ...
  finite.metrics = {
    ...
    staticVersionMetrics = ''
      finite_component_build_info{host="finite-lat-4",component="finite-saas-runner",version="${finitePackages.finite-saas-runner.version}",git_sha="${revision}",image_digest="",source="nix"} 1
      finite_component_version_mismatch{host="finite-lat-4",component="finite-saas-runner"} 0
      finite_component_build_info{host="finite-lat-4",component="nixos-system-profile",version="${config.system.nixos.version}",git_sha="${revision}",image_digest="",source="nix"} 1
      finite_component_version_mismatch{host="finite-lat-4",component="nixos-system-profile"} 0
    '';
  };
  ...
  assertions = [
    {
      ...
      message = "finite-lat-4 arrays must be created and booted with the pinned Linux 6.18 NixOS 26.05 kernel";
    }
    {
      assertion = config.fileSystems."/".device == "/dev/md/root";
      message = "finite-lat-4 root must be the named root MD array";
    }
    {
      assertion = config.fileSystems."/data".device == "/dev/md/data";
      message = "finite-lat-4 /data must be the named data MD array";
    }
    {
      assertion =
        config.systemd.services.finite-saas-runner.environment.FINITE_IDENTITY_AUTHORITY
        == identityAuthority;
      message = "finite-lat-4 Runner must use the private production Identity Authority";
    }
    {
      ...
      message = "finite-lat-4 Runner must load its Identity Authority operator credential";
    }
  ];
  ...
  systemd.network.networks = {
    "10-wan" = {
      matchConfig.MACAddress = "90:5a:08:2e:65:df";
      address = [
        "152.236.34.15/31"
        "2605:6440:2004:1ac::2/64"
      ];
      routes = [
        { Gateway = "152.236.34.14"; }
        { Gateway = "2605:6440:2004:1ac::1"; }
      ];
      networkConfig = {
        DHCP = "no";
        IPv6AcceptRA = false;
    ...
    "20-unused-lan" = {
      matchConfig.MACAddress = "90:5a:08:2e:65:de";
      networkConfig = {
        DHCP = "no";
        IPv6AcceptRA = false;
  ...
  # The BMC's ASPEED adapter owns the host console. The unused Raphael-based
  # EPYC iGPU has no firmware on this headless runner and otherwise logs fatal
  # amdgpu initialization errors on every boot.
