...
in
{
  systemd.services = {
    finite-prepare-data-root = {
      description = "Prepare bounded finite-lat-5 data roots on the exact data filesystem";
      requires = [ "data.mount" ];
      after = [ "data.mount" ];
    ...
    finite-storage-health = {
      description = "Fail-closed finite-lat-5 storage, ESP, swap, and SMART health";
      after = [
        "local-fs.target"
        "swap.target"
      ];
    ...
    finite-md-check = {
      description = "Serialized finite-lat-5 MD consistency check";
      serviceConfig = {
        Type = "oneshot";
