...
- The exact reviewed tools completed a real-data rehearsal against an isolated
  ...
  media-path result, and cleanup outcome. A synthetic test is not this gate.
...
running container image ID. Then require no Austin pod while its PVC stays
Bound:
...
### 4. Export sessions from the frozen PVC
...
its hash before extracting. The single source launcher accepts only the two
export commands and reuses the source image's immutable Hermes wrapper
environment instead of the mutable user venv.
...
Then run the container as root so it can read the frozen PVC and write only
the root-owned staging directory.
...
must report 80 facts. Retain both commands' counts, byte counts, and SHA-256
output.
...
the compatibility summary. It must say legacy skills are review-only, the
Brain working tree must be freshly authorized and synced, and how many session
paths were rewritten, how many cache-media paths remain archive-only, and how
many other legacy paths remain unmapped.
...
Require all VERIFY checks below. Keep box1 at zero replicas and keep both
