#!/usr/bin/env python3
"""Least-active streaming router for local Inkling-Small replicas."""
...
MAX_ACTIVE_PER_BACKEND = int(
    os.environ.get("INKLING_MAX_ACTIVE_PER_BACKEND", "256")
)
...
async def proxy(request: web.Request) -> web.StreamResponse:
    ...
    for _ in range(len(BACKENDS)):
        ...
        # Never replay after an upstream response begins. A replay could
        # duplicate streamed output or tool side effects.
        try:
            backend.served += 1
            response_headers = {
                key: value
    ...
    raise web.HTTPBadGateway(text=f"all Inkling replicas unavailable: {last_error}")
...
async def start_client(app: web.Application) -> None:
    app["client"] = ClientSession(
        ...
        timeout=ClientTimeout(total=None, connect=10, sock_read=1200),
    )
