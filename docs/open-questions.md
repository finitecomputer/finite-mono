...
## One chat product across Hosted and Electron Devices (resolved 2026-07-11)
...
- 2026-07-11: Resolved with one React `ChatProduct`, controller, product stylesheet, and typed `ChatTransport` contract in `@finite/chat-ui/react`. Hosted Web is now an authenticated HTTP/EventSource adapter and Electron is an IPC/local-daemon adapter; neither host owns a transcript, composer, tool renderer, or conversation state machine.
- 2026-07-11: A later-linked or pre-alpha Device receives a target-bound, bounded MLS-encrypted bootstrap of the canonical Room/Topic/Chat metadata and current transcript. Existing pairings request the same repair inside the Room without deleting local identity or making the server a plaintext-history authority.
