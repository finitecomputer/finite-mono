...
## Dashboard Finite Sites list/share authority (owner-viewing path resolved 2026-07-14)
...
A real solution must: name whether the surface lists Project Repositories, Project Outputs, or both; identify the owning and acting Principal; distinguish creation of a requesting-user grant from exchange of an existing grant for a Viewer Cookie; decide whether the Core Project owner's verified Account Auth email or enrolled human Native Principal becomes the default revocable viewer of Outputs published by that Project's Agent Principal; name which Principal authorizes and revokes that grant; use Finite Sites' existing Project List, Visibility, Share, and delegation contracts; preserve revocation; and keep publishing plus sharing independent from Runtime lifecycle.
...
- 2026-07-14: Resolved for Agent-published Outputs by ADR 0004 and Sites ADR 0025. The Agent Principal's signed Project Init atomically creates an explicit Native Principal Share for the authenticated human sender npub. The bounded Sites adapter proves the same User Nostr Identity and exchanges an existing Share for Viewer Cookies; it cannot create authority. Agent and human remain separate Principals, and Share revocation is checked on every content request. Project/Output listing beyond this publish-and-view path remains separate product work.
