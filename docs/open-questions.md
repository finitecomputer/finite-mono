...
- 2026-07-13: Paul accepted the server-held **User Nostr Identity** already used by Hosted Web Chat as Hosted Web Brain's acting **Member Identity**. This is an explicit trusted-hosted trade-off: the web experience is not browser E2EE or operator-blind, and existing Vault access attached to another Member Identity still requires explicit regrant or recovery. This resolves which Principal acts in Hosted Web, but not whether browser JavaScript receives the nsec or uses a constrained signer/decrypter bridge, how that session is isolated, or how existing Folder Key Grants are delivered and recovered. The cross-platform recommendation and questions for Austin are in [Brain and Sites Integrated Viewing Across Web, Electron, and iOS](brain-sites-integrated-viewing-decision.md).
