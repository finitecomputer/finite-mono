...
Status: PAUSED (2026-07-24 — Paul explicitly held Phala work because the
current path remains too work-in-progress)
...
customer admission. On 2026-07-24 Paul paused this run as well. Preserve the
queue and the one existing canary, but perform no further provider mutation,
spend, rollout, or acceptance work until he explicitly reactivates it.
...
PAUSED status removes work authority. Paul's 2026-07-23 instruction authorized
the one existing paid internal `tdx.medium` launch-code canary owned by
`paul@finite.vip`, but the 2026-07-24 pause supersedes further
create/start/restart/stop, provider-console mutation, credential installation,
deployment, acceptance, or spend. A later restart requires an explicit
reactivation and still does not authorize a second paid resource, provider
delete, Stripe Product or Price activation, customer admission, recovery-key
custody changes, or a higher cap.
