...
- 2026-07-14 — There is no deletion/archive path for a hosted (non-imported) Agent: the dashboard has no delete UI and Core's archive endpoint requires `import_candidate_id`. Sol 2 removal is blocked on this; define owner-initiated Agent deletion (entitlement, runtime teardown, data retention semantics) as a deliberate run item rather than manual binding surgery.
