...
- 2026-07-14 — Hosted Agent removal remains unavailable. An operator-only UI and Core offboarding transition now exist, but Core policy and every current Runner advertise Runtime Retirement as false. Define and prove the capability, entitlement release, retained-data policy, and failure recovery before enabling it; do not bypass the gate with manual container or binding changes.
...
- 2026-07-15 — After first-cohort training, run the separately proposed per-Agent `/data` backup, empty-target restore, missing-canonical alert, and generic relaunch work recorded in [`production-baseline-2026-07-15.md`](production-baseline-2026-07-15.md). It is intentionally not part of the training-day checkpoint.
