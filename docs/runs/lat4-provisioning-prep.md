...
| 2026-08-28 | **Agent-state transfer plan agreed with Paul (dumbpipe → operator Mac → lat4).** Inbound: (1) `finite-saas-runner.tar.zst` 19 GB, sha256 `a839a582eb5b48fee08a8971c4d9d4cb585adedc3e715b8cd94ef9699d7fe4ba` — the complete lat1 `/var/lib/finite-saas-runner` tree (154,299 files; active/retired/rollback/upgrade-snapshot/stranded-repair state for every existing Runtime), pulled read-only from lat1's NVMe in rescue mode 2026-08-28 post-thermal-crash, quiescent; **only copy in existence — no off-box backup covered this tree**; per-file `runner-files.sha256` (154,299 lines) + `runner-files.sizes` ride alongside. (2) `/data/finite-saas-runner/kata` — 13 GB second Kata runtime area. (3) `runtime_95aa8aa8937bf5c32922.tar.zst` — 7.7 GB Recovery Set archive (Aug 26 recovery op). NOT inbound by design: chat/brain/identity/Postgres/Core/sites/secrets (those restore on lat2). Tickets are one-shot; 3 MB test first, one fresh ticket per archive. Verify flow: archive sha256 → extract → `sha256sum -c runner-files.sha256`. Restore gate from the capacity run: writer stopped, containerd task absent, stable manifests → empty target → PRAGMA integrity_check + identity-hash equality. Open item: archive root is `finite-saas-runner/` (lat1 workRoot `/var/lib/...`); our lat4 declares `workRoot = /data/finite-saas-runner` (lat3-style, on the 1.8T data array) — `tar -C /data` lands the tree exactly there; confirm with Paul that extracting under `/data` is acceptable for the Core re-point instead of `/var/lib` (root array is small) | §5 Gate F |
