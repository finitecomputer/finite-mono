...
| 2026-08-28 | Gate A executed from this branch: SMART PASSED on all four NVMe devices (smartmontools installed on the throwaway interim OS); capture re-run clean — 2 root-class + 2 data-class disks, none undersized; EUI→device mapping matches the committed `storage-ids.nix`. Capture-script parsing bug fixed (Samsung model strings contain spaces; the same bug exists in #715's `capture-lat2-host-evidence`). Remaining Gate A item: provider console confirmation (static IP, rescue mode) | §5 Gate A |
| 2026-08-28 | **lat1 evacuation package delivered to the operator Mac and fully hash-verified** (dumbpipe, Paul sending): `finite-saas-runner.tar.zst` 20,089,881,363 B = `a839a582…` ✅; `data-finite-saas-runner.tar.zst` 8,189,970,726 B = `df451a5c…` ✅; `runtime-recovery-archive.tar.zst` 8,193,943,598 B = `6c3c1e53…` ✅; `manifests-bundle.tar` 50,765,312 B = `bf9a1119…` ✅ (contains `runner-files.sha256` 154,299 lines, `runner-files.sizes`, `data-kata-files.sha256`, `data-kata-files.sizes`). Transfer quirks solved: dumbpipe 0.39 emits a 156-byte iroh teardown line to stdout on the receiver side (the `head -c <exact>` pattern absorbs it; tiny streams need sender-side padding to survive setup/teardown); one mid-stream drop was closed with a tail-resume ticket. The full lat1 agent-state evacuation is now staged on the Mac awaiting Gates C and F on lat4 | §5 Gate F |
| 2026-08-29 | **Gate F import phase executed on lat4; relocation remains pending.** All eight files copied Mac→lat4 and full archive hashes matched. Main capture then newer kata overlay produced 154,299 manifest-clean files under `/data/finite-saas-runner`; 217 SQLite scratch-copy integrity checks passed; imported directory and manifest inventories agree on 35 payload durable-state IDs; every identity file is regular; deployed Runner target state manifests recorded root-only in `/data/staging/lat4-target-state-manifests.txt`. Runner remains condition-gated and inactive with zero containers/tasks. WireGuard and private Core reachability are live. Paul still owns the Core-record intersection and exact per-Runtime relocation/observation phase. | §5 Gate F |
| 2026-08-29 | **Gate D credentials, undrain, and exact relocation of the active lat1 set completed by Paul.** Lat4 runs 21 `finite` namespace tasks, undrained; relocated Runtimes recorded on `2026-08-29.1`. Product acceptance is blocked by the hermes sidecar wedge (PR #765) and Finite Private allowlist (one key) while `finite.computer` is the Vercel outage page. Core usage API is still on lat2 (`64.34.80.19`) if resolved directly; do not switch the limiter to `usage-api` against the outage origin. Host new-launch pin still `2026-08-27.2`. | §5 Gate D/F |
