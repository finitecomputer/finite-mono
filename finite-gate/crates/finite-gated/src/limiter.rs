...
impl PublicRouteLimiter {
    ...
    pub fn check(&self, ip: IpAddr) -> bool {
        ...
        match windows.get_mut(&ip) {
            ...
            _ => {
                if windows.len() >= self.max_entries && !windows.contains_key(&ip) {
                    return false;
                }
...
mod tests {
    ...
    #[test]
    fn full_bucket_table_fails_closed_for_new_clients() {
        let mut limiter = PublicRouteLimiter::new(2, 60);
        limiter.max_entries = 1;
        let existing = "192.0.2.1".parse().unwrap();
        assert!(limiter.check(existing));
