...
impl NostrEvent {
    ...
    pub fn compute_id(
        ...
        if !hex::is_hex32(pubkey) {
            return Err(AuthnError::InvalidEvent("pubkey must be 32-byte hex"));
        }
    ...
    pub fn sign(
        ...
        tags: Vec<Vec<String>>,
        content: String,
    ) -> Result<NostrEvent, AuthnError> {
        let secp = Secp256k1::new();
        let keypair = Keypair::from_seckey_slice(&secp, secret_key)
            .map_err(|_| AuthnError::InvalidEvent("invalid secret key"))?;
    ...
    pub fn verify(&self) -> Result<&str, AuthnError> {
        ...
        if expected_id != self.id {
            return Err(AuthnError::InvalidEvent("id does not match fields"));
        }

        let pubkey_bytes = hex::decode32(&self.pubkey)?;
        let xonly = XOnlyPublicKey::from_slice(&pubkey_bytes)
            .map_err(|_| AuthnError::InvalidEvent("pubkey is not a valid x-only point"))?;
        let sig_bytes = hex::decode(&self.sig)?;
        let sig = Signature::from_slice(&sig_bytes).map_err(|_| AuthnError::InvalidSignature)?;
        ...
        if secp.verify_schnorr(&sig, &message, &xonly).is_ok() {
            ...
            Err(AuthnError::InvalidSignature)
        }
    }
...
pub fn pubkey_for_secret(secret_key: &[u8; 32]) -> Result<String, AuthnError> {
    let secp = Secp256k1::new();
    let keypair = Keypair::from_seckey_slice(&secp, secret_key)
        .map_err(|_| AuthnError::InvalidEvent("invalid secret key"))?;
    let (xonly, _parity) = keypair.x_only_public_key();
...
mod tests {
    ...
    fn tampered_content_fails_id_check() {
        ...
        assert_eq!(
            event.verify(),
            Err(AuthnError::InvalidEvent("id does not match fields"))
        );
    }
    ...
    fn tampered_sig_fails_signature_check() {
        ...
        event.sig = other.sig;
        assert_eq!(event.verify(), Err(AuthnError::InvalidSignature));
    }
