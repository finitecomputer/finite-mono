...
use crate::control::{ConnectionControl, ControlRequest};
            ...
            _ => {
                self.control
                    .execute(&ControlRequest {
                        ...
                        schema: request.body.schema.clone(),
                        body: serde_json::from_slice(&request.body.json_payload).map_err(|_| {
                            AgentdError::InvalidPayload(
                                "request JSON did not match its schema".to_owned(),
                            )
                        })?,
                    })
                    .await
